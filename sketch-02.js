...
function draw() {
  ...
  if (poses.length > 0) {
    ...
    let noseY = poses[0].pose.keypoints[0].position.y; // Nariz
    let rightShoulderY = poses[0].pose.keypoints[6].position.y; // Hombro derecho
    let leftShoulderY = poses[0].pose.keypoints[5].position.y; // Hombro izquierdo
